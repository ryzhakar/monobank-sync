    ...
    for token_index in 0..tokens.len() {
        ...
        for raw_account in raw_client_info.accounts {
            ...
            let account = models::Account {
                ...
                last_sync_at: Some(last_sync_time.clone()),
            };
            let account_clone = account.clone();
            let _ = executor.execute(
                move |p| crud::insert_account(p.clone(), account_clone)
            );
            ...
            for statement_response in card_statements {
                ...
                match statement_response {
                    Ok((timestamp, s)) => {
                        ...
                        let _ = executor.execute(
                            move |p| crud::update_last_sync_time(
                                p.clone(),
                                account_id,
                                last_success,
                            )
                ...
                let statements = raw_statements.into_iter().map(
                    |s| models::StatementItem {
                        id: s.id,
                        account_id: account.id.clone(),
                        time: s.time,
                ...
                for statement_item in statements {
                    let _ = executor.execute(
                        move |p| crud::insert_statement_item(p.clone(), statement_item)
                    );
                };
            }
        }

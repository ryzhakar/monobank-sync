...
This project uses three workflows with two complementary tools:

**Workflows:**
- `ci.yml` - Validates code and creates release PRs
- `publish.yml` - Publishes to crates.io and creates tags
...
These are separate because they solve different problems and trigger on different events. release-plz handles version management and source distribution, cargo-dist handles binary distribution. Both are standard tools in the Rust ecosystem.
...
Each workflow has a distinct trigger and purpose:
- `ci.yml` triggers on push to main and PR updates
- `publish.yml` triggers only on PR merge
- `release.yml` triggers only on tag creation

This separation makes each workflow simple and explicit about when it runs. No complex conditionals needed.
...
## Workflows and Jobs Reference

### ci.yml - Continuous Integration

**Trigger:** Push to main, PR updates
...
#### create-release-pr
**When:** After validate passes, only on push to main (not on PRs)
**What it does:** Analyzes commits since last release, determines version bump via cargo-semver-checks, updates Cargo.toml and CHANGELOG.md, creates/updates PR
...
#### publish-and-tag
**What it does:** Publishes crate to crates.io, creates git tag (does NOT create GitHub release)
**Why it exists:** Automated publishing with the tag serving as the handoff point to cargo-dist
**Note:** GitHub releases are created by cargo-dist, not release-plz (configured via `git_release_enable = false`)

### release.yml - Binary Distribution (auto-generated by cargo-dist)

**Trigger:** Version tag is created
**Purpose:** Multi-platform binary distribution

**Jobs:**
- **plan** - Determines what to build
- **build-local-artifacts** - Builds binaries for 5 platforms in parallel
- **build-global-artifacts** - Generates checksums and source tarball
- **host** - Creates GitHub Release and uploads all artifacts
- **announce** - Placeholder for future announcements

**Why it exists:** Users can install pre-built binaries instead of compiling from source
**Important:** This file is auto-generated. Never edit manually. Regenerate with `dist generate`.
...
**Important:** Version bumps are determined by **cargo-semver-checks**, not just conventional commits!

cargo-semver-checks analyzes actual API changes:
- Breaking API changes → major (0.2.0 → 1.0.0)
- New public API → minor (0.2.0 → 0.3.0)
- Bug fixes, internal changes → patch (0.2.0 → 0.2.1)

Conventional commits guide the analysis but the actual API diff is authoritative. A `feat:` commit with no new public API results in a patch bump, not a minor bump.
...
The `release-plz.toml` file in the project root configures release-plz behavior:
...
This tells release-plz to NOT create GitHub releases - cargo-dist handles that. release-plz only publishes to crates.io and creates tags.

### Important: release.yml is auto-generated

Never edit `.github/workflows/release.yml` manually. It's generated by cargo-dist and excluded from YAML formatting hooks. To modify it, change `dist-workspace.toml` and run `dist generate`.
